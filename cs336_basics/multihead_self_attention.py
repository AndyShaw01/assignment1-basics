...
class MultiHeadSelfAttention(torch.nn.Module):
    def __init__(self, d_model, num_heads, max_seq_length, device=None):
        super().__init__()
        assert d_model % num_heads == 0, "d_model must be divisible by num_heads"
        self.d_model = d_model
        self.num_heads = num_heads
        self.head_dim = d_model // num_heads # keep "int"
        self.w_q = Linear(self.d_model, self.d_model, device=device)
        self.w_k = Linear(self.d_model, self.d_model, device=device)
        self.w_v = Linear(self.d_model, self.d_model, device=device)
        self.w_o = Linear(self.d_model, self.d_model, device=device)
